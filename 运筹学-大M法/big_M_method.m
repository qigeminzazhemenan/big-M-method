...
basis_loc = repelem(0,m);%初始基变量的位置(非最终)
t1 = 1;%t1辅助索引
...
basis_loc = basis_loc+n;%初始基变量的位置
basis_labels = label_list(basis_loc);%初始基变量的名称
Cb = c(basis_loc);%初始基变量在目标函数中的系数
...
%构建初始单纯形表
table = horzcat(Cb',basis_labels',b',A,xita);
...
while max(L*max_or_min)>0
    ...
    [~,out_label] = min(xita);
    out_label = basis_labels(out_label);%出基变量的名称
    out_loc = find(label_list==out_label);%出基变量的位置
    basis_loc(basis_loc==out_loc) = [];
    basis_labels(basis_labels==out_label) = [];%出基
    basis_loc(m) = enter_loc;
    basis_labels(m) = enter_label;%入基
    
    %计算新一轮的系数，检验数，西塔值
    T = A(:,basis_loc);%T辅助矩阵,用于计算新的A与b
    A = T\A;
    b = (T\b')';
    Cb = c(basis_loc);
    L = c-Cb*A;
